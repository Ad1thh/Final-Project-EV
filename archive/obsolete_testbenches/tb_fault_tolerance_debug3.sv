// ============================================================================
// File: tb_fault_tolerance_debug3.sv
// Description: Minimal debug with timeouts
// ============================================================================

`timescale 1ns/1ps

module tb_fault_tolerance_debug3;
    ...
    initial begin
        ...
        $display("========================================");
        $display("DEBUG 3: Timeout-Protected Fault Tests");
        $display("========================================");

        // --------------------------------------------------------------------
        // Test A: SEC-DED fault injection with timeout
        // --------------------------------------------------------------------
        $display("\n--- Test A: SEC-DED fault ---");

        mem[0] = 32'hDEAD02B7; // lui x5, 0xDEAD
        mem[1] = 32'h00B08133; // addi x5, x5, 0xBEF
        mem[2] = 32'h00000073; // ebreak
        
        // Wait for trap with timeout
        fork
            begin: trap_wait
                wait(trap);
                disable timeout;
            ...
            begin: timeout
                #5000;
                $display("  TIMEOUT waiting for trap");
                disable trap_wait;
            end
        join

        @(posedge clk);
        $display("  After program: trap=%b, pc=%h", trap, pc_debug);

        // Inject fault into x5, bit 0
        @(posedge clk);
        fi_reg_en = 1'b1;
        fi_reg_addr = 4'd5;
        fi_reg_bit = 6'd0;
        ...
        // --------------------------------------------------------------------
        // Test B: TMR ALU fault with timeout
        // --------------------------------------------------------------------
        $display("\n--- Test B: TMR ALU fault ---");
        ...
        // Let ADD reach EX stage
        @(posedge clk); @(posedge clk); @(posedge clk);
        
        $display("  About to inject fault: pc=%h, trap=%b", pc_debug, trap);
        ...
        $display("  After ALU fault:");
        $display("    tmr_mismatch=%b", tmr_mismatch);
        $display("    trap=%b", trap);

        // Check result in memory
        $display("  memory[0] = %h (expected 0000000F if ADD 5+10 correct)", mem[0]);

        $display("\n========================================");
        $display("DEBUG 3 COMPLETE");
        $display("========================================");
